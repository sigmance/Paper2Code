# coding: utf-8
"""Gemini-based logic analysis stage."""
import json
import os
import sys
import argparse
...
model = genai.GenerativeModel(args.gemini_version)
...
for todo_file_name in tqdm(todo_file_lst):
    responses = []
    trajectories = copy.copy(analysis_msg)
    ...
    trajectories.append({'role': 'assistant', 'content': completion_json['choices'][0]['message']['content']})
    ...
    with open(f'{output_dir}/{save_name}_simple_analysis_response.json', 'w') as f:
        json.dump(responses, f)

    with open(f'{output_dir}/{save_name}_simple_analysis_trajectories.json', 'w') as f:
